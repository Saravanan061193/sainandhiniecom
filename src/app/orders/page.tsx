...
import { ShoppingBag, Package, ChevronRight, Clock, CheckCircle2, ShieldAlert, ArrowRight, Truck } from "lucide-react";
        ...
        <main className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="flex-grow pt-48 pb-32 max-w-5xl mx-auto px-4 sm:px-6 w-full">
                <div className="flex justify-between items-end mb-16">
                    <div>
                        <span className="text-primary font-bold uppercase tracking-[0.3em] text-xs mb-4 block">Order History</span>
                        <h1 className="text-5xl font-serif font-bold text-primary-dark">Your Past <span className="text-primary italic">Delights</span></h1>
                    </div>
                    <div className="hidden md:block">
                        <p className="text-gray-400 font-medium text-sm text-right">Showing {orders.length} orders from<br />your lifetime collection.</p>
                    </div>
                </div>
                ...
                {orders.length === 0 ? (
                    <div className="bg-white p-20 rounded-[4rem] text-center shadow-xl border border-gray-100">
                        ...
                        <h3 className="text-3xl font-serif font-bold text-primary-dark mb-4">No orders yet.</h3>
                        <p className="text-gray-500 mb-10 max-w-sm mx-auto font-medium">Your basket is waiting to be filled with authentic South Indian flavors.</p>
                        <Link href="/shop" className="bg-primary text-white px-10 py-5 rounded-2xl font-bold shadow-xl hover:bg-primary-dark transition-all inline-flex items-center gap-3">
                                ...
                                <div className="flex flex-col md:flex-row justify-between gap-10">
                                    <div className="flex-grow">
                                        <div className="flex items-center gap-4 mb-6">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-serif font-bold text-primary-dark mt-1">#{order._id.slice(-8).toUpperCase()}</p>
                                                {order.awbNumber && (
                                            ...
                                            <div className="ml-auto md:ml-6 flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-full">
                                                {order.status === 'Delivered' ? <CheckCircle2 size={14} className="text-green-500" /> :
                                                    order.status === 'Shipping' ? <Truck size={14} className="text-blue-500" /> :
                                                        order.status === 'Processing' ? <Package size={14} className="text-orange-500" /> :
                                                            <Clock size={14} className="text-gray-400" />}
                                                <span className={`text-[10px] font-bold uppercase tracking-widest 
                                                    ${order.status === 'Delivered' ? 'text-green-500' :
                                                        order.status === 'Shipping' ? 'text-blue-500' :
                                                            order.status === 'Processing' ? 'text-orange-500' : 'text-gray-500'}`}>
                                        ...
                                        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                                            {order.orderItems.map((item: any, idx: number) => (
                                                <div key={idx} className="flex-shrink-0 text-center">
                                                    ...
                                                    <p className="text-[10px] font-bold text-primary-dark line-clamp-1 w-20">x{item.qty} {item.name}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col justify-between items-end md:w-48 border-t md:border-t-0 md:border-l border-gray-50 pt-8 md:pt-0 md:pl-10">
                                        <div className="text-right">
                                            ...
                                            <p className="text-3xl font-bold text-primary mt-1">₹{order.totalPrice.toLocaleString()}</p>
                                            <p className={`text-[10px] font-black uppercase tracking-widest mt-2 ${order.isPaid ? 'text-green-500' : 'text-orange-400'}`}>
