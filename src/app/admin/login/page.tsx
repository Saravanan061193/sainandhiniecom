    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res?.error) {
                ...
                // In a real app, NextAuth would verify the role in the authorize callback
                // We'll redirect to admin dashboard and let the middleware/layout handle role check
                ...
                <div className="bg-white rounded-[2.5rem] p-10 shadow-2xl border border-white/20">
                    <div className="text-center mb-8">
                        <Link href="/" className="inline-flex items-center gap-3 mb-6">
                            <div className="w-12 h-12 bg-[#556B2F] rounded-xl flex items-center justify-center text-white font-black text-2xl shadow-lg">SN</div>
                            <span className="text-2xl font-serif font-black text-[#556B2F]">Tasty World</span>
                        </Link>
                        <div className="flex items-center justify-center gap-2 mb-2">
                            <ShieldCheck className="text-[#556B2F]" size={20} />
                            <h2 className="text-xl font-black text-[#556B2F] uppercase tracking-widest">Admin Portal</h2>
                        </div>
                        <p className="text-gray-400 text-sm font-medium">Access your management dashboard</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-2">Admin Email</label>
                            <input
                                type="email"
                                required
                                value={email}
                                onChange={(e) => setEmail(e.target.value)}
                                className="w-full bg-[#fcf9f4] border-2 border-transparent focus:border-[#556B2F]/10 focus:bg-white rounded-2xl py-4 px-6 outline-none transition-all font-bold text-[#556B2F]"
                        ...
                        >
                            {loading ? <Loader2 className="animate-spin" size={20} /> : (
                                <>Verify Admin Access <ArrowRight className="group-hover:translate-x-1 transition-transform" size={18} /></>
                            )}
                        </button>
                    </form>

                    <div className="mt-10 pt-8 border-t border-gray-100 text-center">
                        <Link href="/login" className="text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-[#556B2F] transition-all">
