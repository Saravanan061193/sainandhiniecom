    ...
    return (
        <div className="space-y-10">
            <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-serif font-bold text-primary-dark">Customer Database</h1>
                    <p className="text-gray-400 mt-1 font-medium">Manage your relationships and view customer lifetime value.</p>
                </div>
                ...
                {filtered.map((customer, i) => (
                    ...
                    >
                        <div className="flex items-center gap-5 mb-8">
                            ...
                            <div>
                                <h3 className="text-lg font-serif font-bold text-primary-dark">{customer.name}</h3>
                        ...
                        <div className="space-y-4 mb-8">
                            ...
                            <div className="flex items-center gap-3 text-sm text-gray-500 font-medium">
                                <Phone size={16} className="text-gray-400" /> {customer.phone || 'N/A'}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-8 border-t border-gray-50">
                            <div>
                                ...
                                <div className="flex items-center gap-2 font-bold text-primary-dark">
                                    <ShoppingBag size={16} className="text-primary" /> {customer.orderCount}
                                </div>
                            </div>
                            <div>
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Lifetime Value</p>
                                <p className="font-bold text-primary text-lg">₹{customer.totalSpent.toLocaleString()}</p>
                            </div>
                        </div>

                        <button className="w-full mt-8 py-4 bg-gray-50 rounded-2xl text-[10px] font-bold text-gray-400 uppercase tracking-widest hover:bg-primary/5 hover:text-primary transition-all flex items-center justify-center gap-2 group">
                            View Purchase History <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" />
                        </button>
                    </motion.div>
                ))}
            </div>
            ...
            {filtered.length === 0 && (
                <div className="text-center py-20 bg-white rounded-[3rem] border border-gray-100">
                    ...
                    <p className="text-gray-400 font-bold uppercase tracking-widest text-sm">No customers found matching your search.</p>
                </div>
            )}
        </div>
    );
