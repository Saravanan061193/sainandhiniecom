...
export default function AdminOrdersPage() {
    ...
    const [sourceFilter, setSourceFilter] = useState("Online");
    ...
    const filteredOrders = useMemo(() => {
        return orders.filter(order => {
            ...
            const matchesPayment = paymentFilter === "All" || (paymentFilter === "Paid" ? order.isPaid : !order.isPaid);
            ...
            return matchesSearch && matchesStatus && matchesPayment && matchesDate && matchesSource;
        });
    }, [orders, searchTerm, statusFilter, paymentFilter, sourceFilter, dateRange]);
    ...
    return (
        <div className="space-y-6 pb-20 font-sans">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white p-8 rounded-[32px] shadow-sm border border-gray-100">
                <div>
                    <h1 className="text-4xl font-serif font-black text-[#2F3E2C] tracking-tight">Order Intelligence</h1>
                    <p className="text-gray-400 font-medium mt-1 tracking-wide">Real-time fulfillment and payment tracking.</p>
                </div>
                <div className="flex items-center gap-8 bg-[#F8F6F2] p-4 rounded-2xl">
                    <div className="text-center">
                        <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Volume</div>
                        <div className="text-2xl font-serif font-black text-[#2F3E2C]">₹{orders.reduce((acc, o) => acc + o.totalPrice, 0).toLocaleString()}</div>
                    </div>
                    <div className="h-10 w-px bg-gray-200" />
                    <div className="text-center">
                        <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Active Orders</div>
                        <div className="text-2xl font-serif font-black text-[#C6A75E]">{orders.filter(o => o.status !== 'Delivered').length}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-[24px] shadow-sm border border-gray-100 flex flex-wrap items-center gap-4 sticky top-0 z-30 backdrop-blur-md bg-white/90">
                ...
                <div className="relative flex-grow min-w-[250px] group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#C6A75E] transition-colors" size={18} />
                    <input
                        type="text"
                ...
                {selectedOrders.length > 0 && (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <span className="bg-[#C6A75E] text-[#2F3E2C] px-3 py-1 rounded-full text-xs font-black">{selectedOrders.length}</span>
                            <span className="text-[10px] font-black uppercase tracking-widest">Orders Selected</span>
                        </div>
                        <div className="h-6 w-px bg-white/10" />
                        <div className="flex items-center gap-2">
                            <span className="text-[9px] font-black uppercase tracking-widest text-white/50 mr-2">Mark As:</span>
                            {ORDER_STAGES.map(s => (
                                <button
                                    key={s}
                        ...
                        <div className="h-6 w-px bg-white/10" />
                        <button className="text-white/50 hover:text-white transition-colors" onClick={() => setSelectedOrders([])}>
                            <X size={20} />
                        </button>
                    </motion.div>
                )}
                            ...
                            <tr>
                                <td colSpan={6} className="px-8 py-32 text-center">
                                    <div className="flex flex-col items-center gap-4 opacity-30">
                                        ...
                                        <span className="text-sm font-black uppercase tracking-widest text-[#2F3E2C]">No matching orders found</span>
                                    </div>
                                </td>
                            </tr>
                        ) : (
                                ...
                                >
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-black text-[#2F3E2C] tracking-tight">#{order._id.slice(-8).toUpperCase()}</span>
                                            <span className="text-xs font-bold text-gray-500 mt-1">{order.shippingAddress.fullName}</span>
                                            <div className="flex items-center gap-2 mt-2">
                                                <Calendar size={10} className="text-gray-300" />
                                                <span className="text-[10px] font-bold text-gray-300 uppercase">{new Date(order.createdAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-xs font-bold text-gray-600">{order.orderItems.length} Products</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col gap-1.5">
                                            <span className="text-lg font-black text-[#2F3E2C] tracking-tighter">₹{order.totalPrice.toFixed(2)}</span>
                                            <div className="flex items-center gap-1.5">
                                                {order.isPaid ? <CheckCircle size={10} className="text-green-500" /> : <AlertCircle size={10} className="text-orange-500" />}
                                                <span className={`text-[9px] font-black uppercase tracking-[0.1em] ${order.isPaid ? 'text-green-500' : 'text-orange-500'}`}>
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="relative group/select max-w-[150px]">
                                            <select
                                                ...
                                                className={`appearance-none w-full bg-[#F8F6F2] border-none rounded-xl py-3 pl-4 pr-10 text-[10px] font-black uppercase tracking-widest cursor-pointer hover:bg-white transition-all focus:ring-2 focus:ring-[#C6A75E]/30 ${order.status === 'Delivered' ? 'text-green-600' :
                                                    order.status === 'Shipping' ? 'text-orange-600' :
                                                        order.status === 'Processing' ? 'text-blue-600' : 'text-gray-500'
                                                    }`}
                                            >
                        ...
                        >
                            <div className="flex justify-between items-start mb-12">
                                <div className="space-y-1">
                                    <div className="bg-[#2F3E2C]/10 text-[#2F3E2C] px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest inline-block mb-2">Order Intelligence</div>
                                    <h2 className="text-4xl font-serif font-black text-[#2F3E2C]">#{viewingOrder._id.slice(-8).toUpperCase()}</h2>
                                    <p className="text-gray-400 font-medium text-xs uppercase tracking-widest mt-2">{new Date(viewingOrder.createdAt).toLocaleString()}</p>
                                </div>
                                <button
                                    onClick={() => setViewingOrder(null)}
                            ...
                            <div className="grid grid-cols-2 gap-10 mb-12">
                                <div className="space-y-6">
                                    <div>
                                        <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Shipping Matrix</h4>
                                        <div className="bg-white p-6 rounded-[24px] border border-gray-100 shadow-sm space-y-2">
                                            <p className="font-bold text-[#2F3E2C] text-lg">{viewingOrder.shippingAddress.fullName}</p>
                                            <p className="text-gray-400 text-sm leading-relaxed">{viewingOrder.shippingAddress.address}</p>
                                            <p className="text-gray-400 text-sm">{viewingOrder.shippingAddress.city} - {viewingOrder.shippingAddress.pincode}</p>
                                            <p className="text-[#C6A75E] font-bold text-sm pt-2">{viewingOrder.shippingAddress.phone}</p>
                                        </div>
                                    </div>
                                    <div>
                                        <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Live Log</h4>
                                        <div className="space-y-4">
                                            <div className="flex gap-4">
                                                <div className="w-8 h-8 rounded-full bg-green-50 text-green-500 flex items-center justify-center shrink-0 border border-green-100"><CheckCircle2 size={14} /></div>
                                                <div>
                                                    <p className="text-xs font-bold text-[#2F3E2C]">Order Received</p>
                                                    <p className="text-[10px] text-gray-400 font-medium">{new Date(viewingOrder.createdAt).toLocaleTimeString()}</p>
                                                </div>
                                            </div>
                                            {viewingOrder.isPaid && (
                                                <div className="flex gap-4">
                                                    <div className="w-8 h-8 rounded-full bg-green-50 text-green-500 flex items-center justify-center shrink-0 border border-green-100"><CheckCircle2 size={14} /></div>
                                                    <div>
                                                        <p className="text-xs font-bold text-[#2F3E2C]">Funds Captured</p>
                                                        <p className="text-[10px] text-gray-400 font-medium">{new Date(viewingOrder.paidAt || viewingOrder.createdAt).toLocaleTimeString()}</p>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div className="space-y-6">
                                    <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Manifest</h4>
                                            ...
                                            {viewingOrder.orderItems.map((item: any, i: number) => (
                                                <div key={i} className="p-4 flex gap-4 items-center">
                                                    ...
                                                    <div className="flex-grow">
                                                        <p className="text-[10px] font-black uppercase text-gray-400 truncate">{item.name}</p>
                                                        <p className="text-xs font-bold text-[#2F3E2C]">₹{item.price} × {item.qty}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
            ...
            <AnimatePresence>
                {toast && (
                    <motion.div
                        ...
                        className={`fixed bottom-10 left-10 z-[1000] px-6 py-4 rounded-2xl shadow-2xl border flex items-center gap-3 ${toast.type === 'error' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-[#2F3E2C] text-white border-white/10'
                            }`}
                    >
                        {toast.type === 'error' ? <AlertCircle size={20} /> : <CheckCircle2 size={20} className="text-[#C6A75E]" />}
