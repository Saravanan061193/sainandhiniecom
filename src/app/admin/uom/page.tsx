...
import { Plus, Trash2, Edit2, Loader2, Save, X, Search, MoreVertical, Check, AlertCircle, Package } from "lucide-react";
...
export default function UOMPage() {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/uom/${id}`, {
                method: "DELETE"
            });
    ...
    return (
        <div className="space-y-8 pb-20">
            <header className="flex justify-between items-end">
                <div>
                    <span className="text-primary font-bold uppercase tracking-[0.3em] text-xs mb-2 block">Master Data</span>
                    <h1 className="text-4xl font-serif font-bold text-primary-dark">Unit <span className="text-primary italic">Manager</span></h1>
                </div>
                <button
                    ...
                    <AnimatePresence>
                        {filteredUoms.map((uom) => (
                            ...
                            >
                                ...
                                <h3 className="text-lg font-bold text-primary-dark">{uom.name}</h3>
                                <p className="text-gray-400 text-sm font-medium mb-4">Code: <span className="font-mono text-xs bg-gray-100 px-2 py-1 rounded">{uom.code}</span></p>

                                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold ${uom.isActive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>
                                    <span className={`w-2 h-2 rounded-full ${uom.isActive ? 'bg-green-500' : 'bg-red-500'}`} />
                        ...
                        >
                            <div className="p-8 pb-0 flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-gray-400 text-sm">Define unit details below.</p>
                                </div>
                            ...
                            <form onSubmit={handleSave} className="p-8 space-y-6">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Unit Code</label>
                                    <input
                                        type="text"
                                    ...
                                    />
                                    <p className="text-[10px] text-gray-400 px-1">Unique identifier for this unit (e.g. kg, box, pcs).</p>
                                </div>
                                ...
                                <div className="flex items-center gap-3 py-2">
                                    ...
                                    >
                                        <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-transform ${formData.isActive ? 'left-6' : 'left-1'}`} />
                                    </button>
