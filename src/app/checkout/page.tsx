...
import { MapPin, CreditCard, ChevronRight, CheckCircle2, ShieldCheck, Loader2, ArrowLeft } from "lucide-react";
...
export default function CheckoutPage() {
    ...
    const isCodAvailable = cartItems.every((item) => item.isCodAvailable !== false);
    ...
    const makePayment = async () => {
        if (!address.fullName || !address.email || !address.phone || !address.street || !address.city || !address.pincode) {
        ...
        try {
            ...
            if (!orderRes.ok) throw new Error(dbOrder.error || "Order creation failed");
            ...
            if (!rzpOrder.id) throw new Error("Could not create Razorpay order (ID missing)");
        ...
        return (
            <main className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
                ...
                <div className="text-center space-y-6">
                    ...
                    <h1 className="text-3xl font-serif font-bold text-primary-dark">Your basket is empty</h1>
                ...
                <div className="flex justify-center mb-16">
                    <div className="flex items-center gap-4 w-full max-w-2xl px-10">
                        <div className="flex flex-col items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-primary uppercase tracking-widest">Address</span>
                        </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-100">
                            ...
                            <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2 space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">House No / Street</label>
                                    <textarea
                                        rows={2}
                                        required
                        ...
                        <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-100">
                            ...
                            <div className="space-y-4">
                                <label className={`flex items-center justify-between p-6 rounded-2xl border-2 cursor-pointer transition-all ${paymentMethod === 'prepaid' ? 'border-primary bg-primary/5' : 'border-gray-100 hover:border-gray-200'}`}>
                                    <div className="flex items-center gap-4">
                                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'prepaid' ? 'border-primary' : 'border-gray-300'}`}>
                                            {paymentMethod === 'prepaid' && <div className="w-3 h-3 rounded-full bg-primary" />}
                                        </div>
                                        <div>
                                    ...
                                    <input type="radio" name="payment" className="hidden" checked={paymentMethod === 'prepaid'} onChange={() => setPaymentMethod('prepaid')} />
                                </label>

                                <label className={`flex items-center justify-between p-6 rounded-2xl border-2 cursor-pointer transition-all ${paymentMethod === 'cod' ? 'border-primary bg-primary/5' : 'border-gray-100 hover:border-gray-200'} ${!isCodAvailable ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-primary-dark">Cash on Delivery</p>
                                            <p className="text-xs text-gray-500">{isCodAvailable ? 'Pay when you receive' : 'Not available for items in cart'}</p>
                                        </div>
                                    </div>
                                    <input type="radio" name="payment" className="hidden" checked={paymentMethod === 'cod'} onChange={() => isCodAvailable && setPaymentMethod('cod')} disabled={!isCodAvailable} />
                                </label>
                            </div>
                        </div>

                        <div className="flex gap-6 justify-center text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                            <div className="flex items-center gap-2"><ShieldCheck size={16} className="text-primary" /> Multi-Layer Encryption</div>
                            <div className="flex items-center gap-2"><CheckCircle2 size={16} className="text-primary" /> Verified Origin</div>
                        </div>
                    </div>

                    <aside className="space-y-8">
                        <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 flex flex-col h-fit">
                            <h2 className="text-2xl font-serif font-bold text-primary-dark mb-8">Summary</h2>
                                ...
                                {cartItems.map((item) => (
                                    <div key={item._id} className="flex gap-4">
                                        ...
                                        <div className="flex-grow">
                                            <h4 className="text-xs font-bold text-primary-dark line-clamp-2 leading-tight">{item.name}</h4>
                            ...
                            <div className="space-y-4 mb-10">
                                <div className="flex justify-between text-xs font-medium text-gray-400">
                                    <span>Products</span>
                                    <span className="font-bold text-primary-dark">₹{itemsPrice}</span>
                                </div>
                                <div className="flex justify-between text-xs font-medium text-gray-400">
                                    <span>Packaging & Tax</span>
                                    <span className="font-bold text-primary-dark">₹{taxPrice.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-xs font-medium text-gray-400">
                                    <span>Shipping</span>
                                    <span className={shippingPrice === 0 ? "text-green-500 font-bold" : "font-bold text-primary-dark"}>
