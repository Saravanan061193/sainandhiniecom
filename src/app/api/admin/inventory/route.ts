...
export async function GET(req: Request) {
    try {
        ...
        const transactions = await StockTransaction.find(query).sort({ createdAt: -1 }).limit(100);
...
export async function POST(req: Request) {
    try {
        ...
        const { productId, variantSku, type, quantity, reason, costPerUnit, supplier } = body;
        ...
        if (product.variants && product.variants.length > 0) {
            if (!variantSku) return NextResponse.json({ error: "Variant SKU is required for variant products" }, { status: 400 });

            stockIndex = product.variants.findIndex((v: any) => v.uom === variantSku);
            if (stockIndex === -1) return NextResponse.json({ error: "Variant not found" }, { status: 404 });
