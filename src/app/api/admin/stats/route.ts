...
export async function GET() {
    try {
                    ...
                    today: [
                        { $match: { createdAt: { $gte: today } } },
                        {
                            $group: {
                                ...
                                pos: { $sum: { $cond: [{ $eq: ["$shippingAddress.address", "Counter Sale"] }, "$totalPrice", 0] } },
                                online: { $sum: { $cond: [{ $ne: ["$shippingAddress.address", "Counter Sale"] }, "$totalPrice", 0] } }
                            }
                        }
                    ],
                    month: [
        ...
        // Stock Alerts
        const lowStockProducts = await Product.find({ stock: { $lte: 10, $gt: 0 } }).select("name stock uom image").limit(5);
        const outOfStockProducts = await Product.find({ stock: 0 }).select("name stock uom image").limit(5);
        ...
        for (let i = 0; i < 7; i++) {
            ...
            chartData.push({
                ...
                posAmount: found ? found.pos : 0,
                onlineAmount: found ? found.online : 0,
        ...
        return NextResponse.json({
            stats: {
                revenue: {
                    ...
                    posTotal: statsRev.total[0]?.pos || 0,
                    onlineTotal: statsRev.total[0]?.online || 0,
                    today: statsRev.today[0]?.total || 0,
                    posToday: statsRev.today[0]?.pos || 0,
                    onlineToday: statsRev.today[0]?.online || 0,
