...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    const description = product.seo?.metaDescription || product.description?.substring(0, 160) || "Authentic South Indian Delicacies";
    const keywords = product.seo?.keywords ? product.seo.keywords.split(',').map((k: string) => k.trim()) : [];
