                    ...
                    >
                        <div className="p-8 border-b border-primary/5 flex justify-between items-center mt-20 bg-secondary/10">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-2xl font-serif font-black text-primary-dark">Your Basket</h2>
                                <span className="bg-white text-primary text-[10px] font-sans font-black px-3 py-1 rounded-full uppercase tracking-widest border border-primary/10 shadow-sm">{cartCount} ITEMS</span>
                            </div>
                            <button onClick={onClose} className="w-10 h-10 rounded-full hover:bg-white flex items-center justify-center text-primary/40 hover:text-primary-dark transition-all">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex-grow overflow-y-auto p-8 space-y-8 bg-[#FCFCFA]">
                            {cartItems.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                                    ...
                                    <p className="text-primary/40 font-sans font-black uppercase tracking-widest text-xs">Your basket is empty</p>
                                    <button onClick={onClose} className="text-primary-dark font-serif font-bold text-lg hover:underline decoration-accent underline-offset-4">Start Shopping</button>
                                </div>
                            ) : (
                                cartItems.map((item) => (
                                    <div key={item._id} className="flex gap-6 group">
                                        <div className="w-24 h-24 bg-white rounded-2xl overflow-hidden flex-shrink-0 border border-primary/5 shadow-sm">
                                            <img src={item.image || "https://images.pexels.com/photos/2062426/pexels-photo-2062426.jpeg?auto=compress&cs=tinysrgb&w=100"} className="w-full h-full object-cover" alt={item.name} />
                                        </div>
                                        <div className="flex-grow flex flex-col justify-between py-1">
                                            <div className="flex justify-between items-start mb-2">
                                                <h4 className="font-bold text-primary-dark font-serif text-lg leading-tight line-clamp-2">{item.name}</h4>
                                                <button
                                            ...
                                            <div className="flex justify-between items-end mt-2">
                                                <div className="flex items-center gap-3 bg-white border border-primary/5 rounded-xl px-2 py-1 shadow-sm">
                                                    ...
                                                    <span className="text-sm font-sans font-black w-4 text-center text-primary-dark">{item.qty}</span>
                                                    <button
                                                        onClick={() => updateQty(item._id, item.qty + 1)}
                                                        className="w-6 h-6 flex items-center justify-center rounded-lg hover:bg-secondary/20 text-primary/40 hover:text-primary-dark transition-all"
                                                    >
                                                ...
                                                <p className="font-sans font-black text-primary-dark text-lg">₹{(item.price * item.qty).toLocaleString()}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))
                            ...
                            <div className="p-8 bg-white rounded-t-[3rem] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)] border-t border-primary/5 z-20 relative">
                                <div className="space-y-3 mb-8">
                                    <div className="flex justify-between text-sm text-primary/60 font-sans font-bold">
                                        <span>Subtotal</span>
                                        <span className="text-primary-dark">₹{cartTotal.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between text-sm text-primary/60 font-sans font-bold">
                                        <span>Shipping</span>
                                        <span className="text-accent font-black uppercase text-[10px] tracking-widest">Calculated at Checkout</span>
                                    </div>
                                    <div className="flex justify-between items-end pt-6 border-t border-dashed border-primary/10 mt-6">
                                        <div className="flex flex-col">
                                            <span className="text-xs font-sans font-black uppercase tracking-widest text-primary/40">Total</span>
                                            <span className="text-2xl font-serif font-black text-primary-dark">INR {cartTotal.toLocaleString()}</span>
                                        </div>
                                    </div>
                                </div>
                                <Link
